...
XDG_CONFIG_HOME="$HERE/gareset-config" \
    "$LINUX/whale" "$HERE/wm.tcl" > "$HERE/wm-gareset.log" 2>&1 &
WM=$!
sleep 1.5
...
"$LINUX/whale" "$HERE/probe-focus.tcl"

# The nuke: exactly what Tk does on its implicit LeaveNotify.
"$LINUX/whale" "$HERE/set-pointerroot.tcl"
sleep 1.5

AFTER=$("$LINUX/whale" "$HERE/probe-focus.tcl")
...
"$LINUX/whale" "$HERE/set-pointerroot.tcl"
sleep 1.5
AFTER2=$("$LINUX/whale" "$HERE/probe-focus.tcl")
