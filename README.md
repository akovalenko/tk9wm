...
**Quirks — обходные пути для кривых клиентов, по умолчанию ВЫКЛЮЧЕНЫ**
(`TK9WM_QUIRKS=1` включает; WM печатает своё состояние строкой `WM: quirks
…`). Сейчас под флагом только одно: таймерные повторы синтетического
ConfigureNotify (400 мс и 1.5 с) вдогонку к событийным отправкам (при
manage и на `MapNotify` клиента). Разделение намеренное — обходной путь,
включённый всегда, скрывает главный вопрос: **знаем ли мы нужный момент**
или выигрываем только повтором. Дефолт отвечает честно.
