...
#    x v h f c d r l does), Enter fires the selected action (raise);
...
CA=$!
wait_client "$HERE/wm-key.log" 'первое-окно'
"$LINUX/whale" "$HERE/client.tcl" "второе-окно" 240x120 "#fcaf3e" "" "" 60 &
CB=$!
wait_client "$HERE/wm-key.log" 'второе-окно'
...
key ctrl+n      # -> maximize-v
key n           # -> maximize-h (bare n is free: no hotkey claims it)
key p           # -> maximize-v
key k           # -> maximize
key Escape
key alt+space
