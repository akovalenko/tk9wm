# send-reload.tcl — ask a running tk9wm to re-read its config in place (reset
# every knob to the code defaults, then source the config again). Finds the WM's
# wmcheck window via _NET_SUPPORTING_WM_CHECK on the root and sends it a
# TK9WM_RELOAD ClientMessage; a zero-mask XSendEvent is delivered to
# the window's creator — the WM itself. Nothing is disturbed on the
# desk: same process, same clients, same frames; only the config's own
# effects are swept and re-applied.
#
#   whale-cli send-reload.tcl ?display?
...
set RELOAD    [XInternAtom $dpy TK9WM_RELOAD 0]
...
set b [binary format iux4wuiux4wuwuwuiux4 33 0 1 0 $wmcheck $RELOAD 32]
set ev [cffi::memory frombinary [binary format a192 $b] unsafe]
XSendEvent $dpy $wmcheck 0 0 $ev
XSync $dpy 0
puts "sent TK9WM_RELOAD to wmcheck [format 0x%x $wmcheck]"
