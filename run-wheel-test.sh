...
"$LINUX/whale" "$HERE/wm.tcl" > "$HERE/wm-wheel.log" 2>&1 &
WM=$!
sleep 1.5
...
echo "--- before:"
"$LINUX/whale-cli" "$HERE/probe-pointer.tcl" :88 | tee "$HERE/wheel-before.log"
xdotool click 5
sleep 1
echo "--- after a wheel scroll inside the client:"
"$LINUX/whale-cli" "$HERE/probe-pointer.tcl" :88 | tee "$HERE/wheel-after.log"
xdotool click 1
sleep 1
echo "--- after an ordinary click inside the client:"
"$LINUX/whale-cli" "$HERE/probe-pointer.tcl" :88 | tee "$HERE/wheel-click.log"
